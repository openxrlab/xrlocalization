...
py::dict prior_guided_pnp(const Eigen::Ref<Eigen::Matrix<float, 2, Eigen::Dynamic, Eigen::RowMajor>> points2D,
    ...
    bool success = colpnp::sovle_pnp_ransac(point2D_vec, point3D_vec, camera_model_name, params, qvec, tvec, num_inliers,
                                            error_thres, inlier_ratio, confidence, max_iter, &mask, robustor,
                                            colpnp::WEIGHT_SAMPLE, &priors_vec);
    if (success) {
